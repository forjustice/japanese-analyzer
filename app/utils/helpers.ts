...
// 使用Gemini TTS朗读文本
export async function speakJapaneseWithTTS(text: string): Promise<void> {
  try {
    const url = await getJapaneseTtsAudioUrl(text);
    const audioElement = new Audio(url);
    
    // 添加音频播放错误处理
    audioElement.onerror = () => {
      console.warn('音频播放失败，尝试使用系统朗读');
      speakJapanese(text); // 调用备用朗读方法
    };
...
export async function getJapaneseTtsAudioUrl(text: string): Promise<string> {
  // apiKey is handled by the api service, so it's not passed here.
  const audioUrl = await synthesizeSpeech(text, 'gemini');
  return audioUrl;
}

// 将 Base64 PCM 数据转换为可播放的 WAV URL (已停用)
/*
